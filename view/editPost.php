...
<form action="index.php?action=updatePost&amp;postId=<?= $post->id() ?>" method="post">
    <div><label for="title">Titre : <input id="title" name="title" type="text" value="<?= $post->title() ?>" required></label></div>
    <div><label for="content">Épisode : <textarea id="content" name="content" required><?= $post->content() ?></textarea></label></div>
    <div>
        Publier ?
...
<p><a href="index.php?action=deletePost&amp;postId=<?= $post->id() ?>"><button>Supprimer !</button></a></p>
