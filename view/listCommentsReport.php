...
<?php foreach ($comments as $comment) {?>
    <div>
        <h5 class="page_content report_title"><i class="fas fa-user"></i> <?= htmlspecialchars($comment->pseudo()) ?> a commenté le <?= htmlspecialchars($comment->datePublication()) ?> le chapitre <?= htmlspecialchars_decode($postsManager->get($comment->idPost())->title()) ?></h5>
