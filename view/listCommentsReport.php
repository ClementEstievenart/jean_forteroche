...
<?php foreach ($comments as $comment) {?>
    <div>
        <h4><?= htmlspecialchars($comment->lastName()) ?> <?= htmlspecialchars($comment->firstName()) ?> à commenté le <?= htmlspecialchars($comment->datePublication()) ?> l'épisode <em><a href="index.php?action=getPost&amp;postId=<?= htmlspecialchars($postsManager->get($comment->idPost())->id()) ?>"><?= htmlspecialchars_decode($postsManager->get($comment->idPost())->title()) ?></a></em></h4>
